...
function* userCoursesSaga() {
  try {
    // 1. Firestore se data fetch (Check 'Courses' vs 'courses' in your DB)
    const snap = yield call(getDocs, collection(db, "Courses"));

    if (snap.empty) {
      console.warn("⚠️ Firestore collection 'Courses' is empty!");
      yield put({ type: "USER_COURSES_SUCCESS", payload: [] });
      return;
    }
    ...
    const list = snap.docs.map((doc) => {
      ...
      return {
        id: doc.id,
        plan: data.plan || data.PLAN || "No Plan Name",
    ...
    console.log("✅ Courses successfully loaded into Saga:", list);
    yield put({ type: "USER_COURSES_SUCCESS", payload: list });

  } catch (err) {
    console.error("❌ Saga Error:", err.message);
    yield put({ type: "USER_COURSES_ERROR", payload: err.message });
  }
}


function* userBlogsSaga() {
  try {
    const snap = yield call(getDocs, collection(db, "adminBlogs"));
    ...
    const blogs = snap.docs
      ...
      // 👉 sirf published blogs user ko dikhao
...
export default function* saga() {
  ...
  yield takeLatest(types.USER_COURSES_FETCH, userCoursesSaga);
  yield takeLatest(types.USER_BLOGS_FETCH, userBlogsSaga);
